...
Given(`admin is on the {string} tab`, (tab: string) => {
  ...
  cy.contains(`div[title='Member Reports']`, 'Member Reports', {
    ...
    .click({ force: true })
    .trigger('mouseover');
  cy.contains(`ul[id='member-reports$Menu'] > li:nth-child(3)`, tab, {
    timeout: 30000,
  }).click({ force: true });
});
...
Then(`{string} must be shown on the left side`, (search: string) => {
  cy.get(`div.d-flex.justify-content-between > span`).should(
    'contain.text',
    search,
  );
});
...
Then(`Quick Filter must be shown on the left side`, () => {
  cy.get(`div > span`).should('contain.text', 'Quick Filter');
});
...
When(`admin clicks refresh icon`, () => {
  ...
  cy.intercept(`POST`, `/graphql?ipAddressSumarryReport`).as(`refresh`);
...
When(`admin clicks custom columns icon`, () => {
  ...
  cy.wait(5000);
  cy.get(`#columns-filter-button-ip-address-summary`).click();
});
