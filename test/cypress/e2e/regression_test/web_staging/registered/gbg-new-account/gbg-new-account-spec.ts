...
When(`User logs in using new account`, () => {
  cy.contains(`Login`, { timeout: 50000 }).click();
  cy.get(`input[placeholder='Username']`, { timeout: 30000 })
    .clear()
    .type('hellouranus');
...
//Verify if Newly Created User will see Banner
...
// Verify if Newly Created User will not be able to Deposit
...
Then(`User should be able to see the verification statement`, () => {
  cy.contains(`Verification Required`).should(`exist`);
});

//Verify if Newly Created User will not be able to Withdraw
...
Then(`User should be able to see the Withdrawal modal`, () => {
  ...
  cy.contains(`User must first make a deposit.`).should(`exist`);
});
